...
shared_workload_helm_args() {
  ...
  SHARED_HELM_ARGS=(
    ...
    --set "workload.envs[0].name=TS_SINGLE_ARRAY"
    --set-string "workload.envs[0].value=${_TS_SINGLE_ARRAY:-0}"
    --set "workload.envs[1].name=PARALLEL_COPY_WORKERS"
    --set-string "workload.envs[1].value=${_PARALLEL_COPY_WORKERS:-32}"
    --set "workload.envs[2].name=SKIP_RAMDISK_STAGING"
    --set-string "workload.envs[2].value=${_SKIP_RAMDISK_STAGING:-1}"
    --set "workload.envs[3].name=TS_DRIVER"
    --set-string "workload.envs[3].value=${_TS_DRIVER:-raw}"
